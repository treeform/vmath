...
* **Breaking change:** `isNan(x)` now returns true only for NaN. It previously also returned true for positive and negative infinity. Replace old checks with `not isFinite(x)` when both NaN and infinity should be rejected.
