...
proc inverse(m: JoltMat44): JoltMat44 =
  var mm = m
  joltMat44Inverse(addr mm)

proc rotationOnlyCopy(value: JoltMat44): JoltMat44 =
  var d = cast[array[16, float32]](value)
  d[12] = 0'f32
  d[13] = 0'f32
  d[14] = 0'f32
  d[15] = 1'f32
  result = cast[JoltMat44](d)
...
proc main() =
  ...
  lines.dumpScalar("transform[0, 0]", matA.get(0, 0))
  lines.dumpScalar("transform[0, 1]", matA.get(0, 1))
  lines.dumpScalar("transform[0, 2]", matA.get(0, 2))
  lines.dumpScalar("transform[0, 3]", matA.get(0, 3))
  lines.dumpScalar("transform[1, 0]", matA.get(1, 0))
  lines.dumpScalar("transform[1, 1]", matA.get(1, 1))
  lines.dumpScalar("transform[1, 2]", matA.get(1, 2))
  lines.dumpScalar("transform[1, 3]", matA.get(1, 3))
  lines.dumpScalar("transform[2, 0]", matA.get(2, 0))
  lines.dumpScalar("transform[2, 1]", matA.get(2, 1))
  lines.dumpScalar("transform[2, 2]", matA.get(2, 2))
  lines.dumpScalar("transform[2, 3]", matA.get(2, 3))
  lines.dumpScalar("transform[3, 0]", matA.get(3, 0))
  lines.dumpScalar("transform[3, 1]", matA.get(3, 1))
  lines.dumpScalar("transform[3, 2]", matA.get(3, 2))
  lines.dumpScalar("transform[3, 3]", matA.get(3, 3))
  ...
  let axisMatQuat = getQuaternion(axisMat)
